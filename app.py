...
def streamlit_config():
    ...
    # title and position
    st.markdown(f'<h1 style="text-align: center;">Resume Analyzer AI (Gemini Edition)</h1>',
...
class resume_analyzer:
    ...
    def gemini_ai(google_api_key, text_content, analyze):

        # Configure the Gemini SDK with the user's API key
        genai.configure(api_key=google_api_key)

        # Create the Gemini model directly (no LangChain, no v1beta issues)
        model = genai.GenerativeModel('gemini-2.5-flash')

        # Send the full resume text + analysis task to Gemini
    ...
    def resume_summary():
        ...
        with st.form(key='Summary'):
            ...
            with col1:
                google_api_key = st.text_input(label='Enter Gemini API Key', type='password')
        ...
        if submit:
            if pdf is not None and google_api_key != '':
                try:
                    with st.spinner('Processing...'):
                        ...
                        summary_prompt = resume_analyzer.summary_prompt(query_with_chunks="") # Not used anymore but kept structure

                        summary = resume_analyzer.gemini_ai(google_api_key=google_api_key, text_content=full_text, analyze="Provide a detailed summary of this resume.")
            ...
            if pdf is not None and google_api_key != '':
                try:
                    with st.spinner('Processing...'):
                        ...
                        strength = resume_analyzer.gemini_ai(google_api_key=google_api_key, text_content=full_text, analyze="Analyze the strengths of this resume in detail.")
            ...
            if pdf is not None and google_api_key != '':
                try:
                    with st.spinner('Processing...'):
                        ...
                        weakness = resume_analyzer.gemini_ai(google_api_key=google_api_key, text_content=full_text, analyze="Identify weaknesses in this resume and offer specific suggestions to improve them.")
            ...
            if pdf is not None and google_api_key != '':
                try:
                    with st.spinner('Processing...'):
                        ...
                        job_title = resume_analyzer.gemini_ai(google_api_key=google_api_key, text_content=full_text, analyze="Based on this resume, suggest 5-10 specific job titles and career paths.")
